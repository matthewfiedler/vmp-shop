...
class Icore_Theme_Options {
	...
	public function getPages( $value = null, $firstblank = false ) {
		...
		foreach ( $obj as $item ) {
			$items[$item->$value]['name'] = $item->post_title;
			$items[$item->$value]['value'] = $item->$value;
		}
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
				...
				case 'select':
					...
					if( isset( $value[ 'parameter' ] ) && $value[ 'parameter' ] != "" ) {
						$opt =  $value[ 'name' ] . ':' . $value[ 'parameter' ];
					} else {
						$opt = $value[ 'value' ];
					}
					
						echo '<option class="'.esc_attr( $value['value'] ).'" value="' . esc_attr( $opt ) . '"' . selected( $theme_options[$id], $opt, false ) . '>' . $value['name'] . '</option>';
					}
					echo '</select>';
